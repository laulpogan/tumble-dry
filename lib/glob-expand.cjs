...
/**
 * GLOB-01: Try Node 22+ fs.globSync first for robust expansion.
 * Falls back to our own walkDir + regex matcher for older Node versions.
 */
function expandGlobNative(cwd, pattern) {
  // Node 22+: fs.globSync is available and handles all glob patterns natively.
  if (typeof fs.globSync === 'function') {
    try {
      const results = fs.globSync(pattern, { cwd });
      return results
...
function expandGlob(cwd, pattern) {
  // Try native fs.globSync first (Node 22+)
  const native = expandGlobNative(cwd, pattern);
  if (native !== null) return native;

  // Fallback: manual glob expansion via walkDir + regex
