...
function structuralDriftScore(beforeText, afterText) {
  ...
  // If stripped content is identical, ALL the diff was structural.
  if (beforeStripped === afterStripped) {
    // Measure raw structural delta via bigram dice on originals.
    const raw = 1 - bigramSimilarity(beforeText, afterText);
    return Number(raw.toFixed(3));
  }
  // Otherwise structural component is the portion of raw diff NOT
  // explained by content diff. Approximate: structural = max(0, raw_diff - content_diff).
  const raw = 1 - bigramSimilarity(beforeText, afterText);
  const content = 1 - bigramSimilarity(beforeStripped, afterStripped);
