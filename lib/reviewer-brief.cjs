...
const styleAnchors = require('./code/style-anchors/index.cjs');
 ...
 * The API dispatch backend (removed in v0.9.0) used to split on that marker
 * for prompt caching. The marker is now inert but kept for compatibility.
 *
...
function buildReviewerBrief({
  ...
  runDir,
  // CODE-01/04/05: code-mode branches.
  artifactKind,
  language,
  regions,
}) {
  const reviewerAgent = fs.readFileSync(reviewerAgentPath, 'utf-8');
  const isCode = artifactKind === 'code';
  const voiceBlock = isCode
    ? (language
        ? `_Code artifact — reviewing in ${language} idioms. Voice is not a dimension; do NOT flag issues a linter would catch._`
        : '_Code artifact — do NOT flag issues a linter would catch. Assume linter-clean input._')
    : ((voiceExcerpts && voiceExcerpts.length)
        ? voiceExcerpts.map((e, i) => `### Voice excerpt ${i + 1} (from ${e.file})\n\n${e.text}`).join('\n\n')
        : '_No voice references configured. Review content, not style._');

  const codeModeHeader = isCode
    ? `\n**artifact_kind:** code\n**language:** ${language || 'unknown'}${regions && regions.length ? `\n**polyglot regions:** ${regions.length} (see content for embedded languages)` : ''}\n`
    : '';
...
${list}`;
  ...
  const voiceSectionHeader = isCode
    ? '## Style context (code artifact — linter-catchable issues out of scope)'
...
function buildEditorBrief({
  artifactText, aggregateMarkdown, voiceExcerpts, voiceSource,
  agentPath, roundNumber,
  // CODE-04: when artifact_kind === 'code', swap voice excerpts for
  // a language-specific style anchor loaded from lib/code/style-anchors.
  artifactKind,
  language,
}) {
  const agent = fs.readFileSync(agentPath, 'utf-8');
  ...
  if (artifactKind === 'code') {
    const anchorMd = styleAnchors.markdownFor(language);
...
## Style anchor (binding) — this is the STYLE reference for the redraft
...
${OUTPUT_INSTRUCTION('proposed redraft (proposed-redraft.md — must include the full redrafted code artifact in the final "## Redrafted artifact" section, with fenced code blocks and `<!-- code-file: ... -->` markers preserved verbatim)')}
`;
  }
...
/**
 * Build a first-impression brief for System 1 gut-reaction pass (round 1 only).
 *
 * The first-impression reviewer gets a truncated artifact: first 500 chars +
 * extracted headings + last paragraph. Their job is to tag a gut reaction
 * (hooked, confused, skeptical, bored, excited) — not to analyze deeply.
 *
 * Per runbook.md §2.4 and psychographic-audit.md §3 (System 1 gap).
 */
...
function extractLastParagraph(text) {
  ...
  return paragraphs.length ? paragraphs[paragraphs.length - 1].trim() : '';
}

function buildFirstImpressionBrief({
  artifactText,
  personaSlug: slug,
  personaBlock,
  reviewerAgentPath,
  ...
  const reviewerAgent = fs.readFileSync(reviewerAgentPath, 'utf-8');
  const first500 = (artifactText || '').slice(0, 500);
  const headings = extractHeadings(artifactText);
  const lastPara = extractLastParagraph(artifactText);

  const truncatedArtifact = `## First 500 characters

${first500}${first500.length >= 500 ? '\n\n[...truncated — first impression only...]' : ''}

## Document headings (structure scan)

${headings || '_(no headings found)_'}

## Closing paragraph

${lastPara || '_(no closing paragraph found)_'}`;

  const firstImpressionInstructions = `## FIRST IMPRESSION MODE

You are doing a **FIRST IMPRESSION** scan. You have 30 seconds. This is System 1, not System 2.
...
**Tag your gut reaction** from exactly one of: \`hooked\`, \`confused\`, \`skeptical\`, \`bored\`, \`excited\`.

**Do NOT analyze deeply.** No structured findings, no severity tags, no assumption stress-tests. Just your raw, immediate reaction as your persona. 2-4 sentences maximum.

## Output format

Write \`critique-first-impression-${slug}.md\` in the round dir:

\`\`\`markdown
# First Impression — {Persona Name}

**Gut reaction:** {hooked | confused | skeptical | bored | excited}

{2-4 sentences explaining the reaction in persona voice}
\`\`\``;

  const prefix = `# First Impression Brief — Round ${roundNumber}

${reviewerAgent}

## Artifact (truncated for first-impression scan)
...
${OUTPUT_INSTRUCTION('first-impression critique')}`;
