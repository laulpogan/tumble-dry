...
const { loadRegister, isRegistered } = require('./structural-register.cjs');
...
/**
 * Parse the per-reviewer score from a critique file.
 * Looks for `**Score:** N/10` anywhere in the markdown (usually near the top).
 * Returns null if not found or malformed.
 */
function parseScore(markdown) {
  const m = markdown.match(/\*\*Score:\*\*\s*(\d+(?:\.\d+)?)\s*\/\s*10/i);
  if (!m) return null;
  const n = parseFloat(m[1]);
  return (n >= 1 && n <= 10) ? Math.round(n * 10) / 10 : null;
}

function aggregateRound(critiquePaths, opts = {}) {
  const all = [];
  const perReviewer = {};
  const scores = {};  // reviewer → score (1-10)
  for (const p of critiquePaths) {
    ...
    if (score !== null) scores[reviewer] = score;
    all.push(...findings);
  }
  ...
  const scoreStats = scoreValues.length > 0 ? {
    scores,
    avg: Math.round((scoreValues.reduce((a, b) => a + b, 0) / scoreValues.length) * 10) / 10,
    min: Math.min(...scoreValues),
    max: Math.max(...scoreValues),
    count: scoreValues.length,
  } : { scores: {}, avg: null, min: null, max: null, count: 0 };
...
function renderAggregate(agg, config, roundNumber, opts = {}) {
  // REGISTER-02: load register and subtract acknowledged findings from material count.
  const register = opts.runDir ? loadRegister(opts.runDir) : [];
  const registeredCount = register.length
    ? (agg.clusters || []).filter(c => c.severity === 'material' && isRegistered(register, c.summary)).length
    : 0;
  ...
  if (registeredCount > 0) {
    lines.push(`**Material (excluding registered):** ${(agg.by_severity.material || 0) - registeredCount}`);
  }
  ...
  // REGISTER-02: subtract registered findings from material count for convergence.
  const materialCount = (agg.by_severity.material || 0) - registeredCount;
  ...
  lines.push('');
  // Scores section — per-reviewer + aggregate for regression tracking
  if (agg.scores && agg.scores.count > 0) {
    lines.push(`## Panel Scores (1–10)`);
    lines.push('');
    lines.push(`**Average:** ${agg.scores.avg}/10 | **Min:** ${agg.scores.min} | **Max:** ${agg.scores.max} | **Reviewers scored:** ${agg.scores.count}`);
    lines.push('');
    lines.push('| Reviewer | Score |');
    lines.push('| --- | --- |');
    for (const [reviewer, score] of Object.entries(agg.scores.scores)) {
      lines.push(`| ${reviewer} | ${score}/10 |`);
    }
    lines.push('');
  }
  ...
  // REGISTER-03: surface acknowledged structural findings
  if (register.length > 0) {
    lines.push('## Acknowledged structural findings (not blocking convergence)');
    lines.push('');
    for (const entry of register) {
      lines.push(`- **${entry.finding_summary}** (registered at round ${entry.registered_at_round}, status: ${entry.status})`);
    }
    lines.push('');
  }
...
function aggregateJson(agg, converged) {
  ...
  return {
    ...
    scores: agg.scores || { scores: {}, avg: null, min: null, max: null, count: 0 },
