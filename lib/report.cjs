...
function writeRoundReport(runDir, round) {
  ...
  const summary = converged
    ? `Round ${round} converged on ${slug}. Material findings: ${material}. Minor: ${minor}. Structural: ${structural}. Drift: ${contentDrift}${driftBlocked}.`
    : `Round ${round} surfaced ${material} material finding(s) across ${agg.critique_count || '?'} reviewers on ${slug}. Minor: ${minor}. Structural: ${structural}. Drift: ${contentDrift}${driftBlocked}.`;
...
function writeFinalReport(runDir) {
  ...
  const lines = [
    ...
    '## Per-round summary',
    '',
    '| Round | Material | Minor | Structural | Drift | Converged |',
    '| --- | --- | --- | --- | --- | --- |',
  ];
  for (const r of rounds) {
    ...
    lines.push(`| ${r} | ${mat} | ${min} | ${struct} | ${d} | ${conv} |`);
  }
  lines.push('');
