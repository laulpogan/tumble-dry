...
async function initRun(cwd, artifactPath) {
  ...
  if (!fs.existsSync(workingPath)) {
    ...
    // CODE-01: loader may attach `meta.artifact_kind` and `meta.language`
    // when the source is a code file / directory; persist for brief builders.
    const meta = result.meta || {};
    const artifactKind = meta.artifact_kind
      || (['code', 'code-dir'].includes(result.format) ? 'code' : 'prose');
    fs.writeFileSync(
...
/**
 * BATCH-01/05: initialize a batch run across multiple artifact paths.
 *
 * Batch slug derives from the common parent dir name + a timestamp. All
 * per-file runs live under `.tumble-dry/<batch-slug>/<file-slug>/` so the
 * existing single-file layout (working.md, history/, round-N/) still works
 * for each file unchanged. A shared batch status.json lives at
 * `.tumble-dry/<batch-slug>/status.json`.
 *
 * Returns:
 *   {
 *     kind: 'batch',
 *     batchSlug: string,
 *     batchDir: string,
 *     fileRuns: [{ slug, runDir, artifactAbs, sourceAbs, fileSlug }, ...],
 *   }
 */
async function initBatch(cwd, artifactPaths) {
  if (!Array.isArray(artifactPaths) || !artifactPaths.length) {
    throw new Error('initBatch: at least one artifact path required');
  }
  // Common parent inference
  const abs = artifactPaths.map(p => path.resolve(cwd, p));
  let common = path.dirname(abs[0]);
  for (const a of abs) {
    const d = path.dirname(a);
    while (!d.startsWith(common)) common = path.dirname(common);
  }
  const parentName = path.basename(common) || 'batch';
  const ts = new Date().toISOString().replace(/[-:]/g, '').replace('T', '-').replace(/\..+$/, '').slice(0, 13);
  const batchSlug = `${slugify(parentName)}-${ts}`;
  const batchDir = path.join(cwd, '.tumble-dry', batchSlug);
  const tumbleRoot = path.join(cwd, '.tumble-dry');
  if (!fs.existsSync(tumbleRoot)) {
    try { ensureGitignore(cwd); } catch { /* best effort */ }
  }
  ...
  for (const a of abs) {
    const sourceAbs = a;
    const fileSlug = slugify(path.basename(sourceAbs));
    const runDir = path.join(batchDir, fileSlug);
    const historyDir = path.join(runDir, 'history');
    fs.mkdirSync(historyDir, { recursive: true });
    const workingPath = path.join(runDir, 'working.md');
    const sourceExt = path.extname(sourceAbs).toLowerCase() || '.md';
    ...
    if (!fs.existsSync(workingPath)) {
      ...
      if (!result || result.ok === false) {
        const reason = (result && result.reason) || 'unknown';
        const detail = (result && result.detail) || 'loader returned no result';
        const err = new Error(`failed to load ${sourceAbs}: ${reason} — ${detail}`);
        err.reason = reason;
        err.detail = detail;
        throw err;
      }
      ...
      if (MARKDOWN_LIKE.has(result.format)) {
        ...
        fs.writeFileSync(path.join(runDir, 'ROUNDTRIP_WARNING.md'), roundtripWarning(sourceAbs, result.format), 'utf-8');
        fs.copyFileSync(workingPath, path.join(historyDir, 'round-0-original.md'));
      }
      const meta = result.meta || {};
      const artifactKind = meta.artifact_kind || (['code', 'code-dir'].includes(result.format) ? 'code' : 'prose');
      fs.writeFileSync(path.join(runDir, 'source-format.json'), JSON.stringify({
        format: result.format,
        source_ext: sourceExt,
        warnings: result.warnings || [],
        loaded_at: new Date().toISOString(),
    ...
    fs.writeFileSync(path.join(runDir, 'source.path'), sourceAbs);
    fs.writeFileSync(path.join(runDir, 'artifact.path'), workingPath);
    fileRuns.push({ slug: fileSlug, runDir, artifactAbs: workingPath, sourceAbs, fileSlug });
  }
  ...
  fs.writeFileSync(path.join(batchDir, 'batch.json'), JSON.stringify({
    ...
    files: fileRuns.map(f => ({ slug: f.fileSlug, source: f.sourceAbs, run_dir: f.runDir })),
    created_at: new Date().toISOString(),
  }, null, 2), 'utf-8');

  return { kind: 'batch', batchSlug, batchDir, fileRuns };
}

module.exports = { initRun, initBatch, roundDir, currentRound, writeFinal, slugify, snapshotHistory, ensureGitignore, loadSource };
