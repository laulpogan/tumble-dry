...
4. **Emit `audience.md`** in the round directory with the required format from `agents/audience-inferrer.md`. Keep each persona's name, bio, hiring job, bounce trigger, and load-bearing belief verbatim from the library (adapt names/bios only if the artifact supplies concrete details — e.g., real company name, real stack — that improve specificity without inventing facts).
