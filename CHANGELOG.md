...
- **`personas/real-people/`** — schema + ethics README, opt-outs ledger, catalog index, two reference briefs (Martin Casado, Simon Willison) verbatim from spec.
- **`examples/the-mask-game/`** — two real `--review` outputs against the Slancha pitch brief, generated by this pipeline. Used as fixture exemplars of "what good looks like."
